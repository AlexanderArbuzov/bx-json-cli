...
Парсит JSON из URL или локального файла в потоковом режиме, извлекает элементы с использованием JSON-указателя (опция `--json-pointer`, по умолчанию: `/items`) и хранит каждый элемент как JSON-строку в списке Redis. Очищает целевой ключ Redis перед парсингом, который вы можете задать с помощью опции `--redis-key` (по умолчанию `items`) и устанавливает флаг завершения по успешному окончанию (`complete-key`).
